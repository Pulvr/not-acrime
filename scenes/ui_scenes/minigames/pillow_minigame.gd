...
signal pillow_minigame_ui_deleted
...
func track_mouse_performance(mouse_pos: Vector2) -> void:
	...
	if (
		closest_offset >= (path_length - end_tolerance)
		and mouse_pos.distance_to(end_point) <= end_tolerance
	):
		complete_tracking()
...
func end_minigame():
	...
	pillow_minigame_ui_deleted.emit()
	minigame_ended = true

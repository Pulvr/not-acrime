	...
	rings = [
		{
			"radius": 260.0,
			"width": 12.0,
			"base_speed": deg_to_rad(base_start_speed * randf_range(0.8, 1.2)),
			"gap_size": _get_gap_size(0),
			"rotation": randf() * TAU
		},
		{
			"radius": 230.0,
...
func _process(delta: float) -> void:
	var speed_multiplier = (
		1.0 + (pow(float(current_ring_index), difficulty_exponent) * difficulty_factor)
	)
...
func _draw() -> void:
	...
	for i in range(current_ring_index, rings.size()):
		...
		draw_arc(
			center_pos, ring["radius"], solid_start, solid_end, 64, ring_color, ring["width"], true
		)
...
func _input(event: InputEvent) -> void:
	var is_a_key = (
		event is InputEventKey and event.keycode == KEY_A and event.pressed and not event.echo
	)
	var is_lmb = (
